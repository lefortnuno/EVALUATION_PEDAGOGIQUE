...
function UE() {
  ...
  const loadUE = async () => {
    const response = await axios.get("http://localhost:5000/allUE");
  ...
  const [NOM_UE, setNOM_UE] = useState("");
  ...
  const addUE = (event) => {
    ...
    if (
      NOM_UE === "" || !selectedClasse.value === ""
    ) {
      alert("Tous les champs sont obligatoires");
    } else {

      const message = `NOM_UTILISATEUR: ${NOM_UE}\nClasse: ${selectedClasse.value}`;
      // alert(message);
      axios
        .post("http://localhost:5000/addUE", {
          NOM_UE: NOM_UE,
          classe: selectedClasse.value,
        })
        .then(() => {
  ...
  const rows = userData.map((user) => ({
    ...
    nom: user.NOM_UE,
    niveau: user.classe.NOM_CLASSE,
    parcours: user.classe.parcours.NOM_PARCOURS, 
    Action: (
      <div>
        {user.NOM_UE && (
