...
const addnewElement = async (req, res) => {
    const data = await ElementModel.ElementSchema(req.body);
...
const getElementbyId = async (req, res) => {
    try {
        ...
        const user = await ElementModel.ElementSchema.findOne({ _id: ID }).populate({
            path: 'UE',
            select: 'NOM_UE classe', // Sélectionnez les champs de classe que vous souhaitez
            populate: {
                path: 'classe',
...
module.exports = {
    addnewElement, getallElement, getElementbyId, updateUE, deleteUE, rechercheUE, chargerUEparclasse,
};